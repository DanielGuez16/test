...
class ReportGenerator:
    ...
    def export_to_pdf(self, output_path):
        """Génère le PDF avec Html2Image - méthode synchrone complète"""
        try:
            logger.info("Début génération PDF avec Html2Image")
            
            # ÉTAPE 1 : Capturer les graphiques avec Html2Image
            self.chart_images = self.capture_charts_with_html2image()
            logger.info(f"Graphiques capturés: {len(self.chart_images)} images")
            
            # ÉTAPE 2 : Générer le HTML complet avec TOUT le contenu
    ...
    def capture_charts_with_html2image(self):
        """Capture TOUS les graphiques avec Html2Image"""
        ...
        if not significant_groups or not metier_details:
            logger.warning("Pas de groupes significatifs ou données métier pour les graphiques")
            return {}
        
        logger.info(f"Capture de {len(significant_groups)} graphiques: {significant_groups}")
        ...
        try:
            ...
            hti = Html2Image()
            hti.size = (700, 400)  # Taille optimale pour PDF
            ...
            for i, groupe in enumerate(significant_groups):
                try:
                    # Générer HTML spécifique pour ce graphique
                    ...
                    if screenshot_files:
                        ...
                        if os.path.exists(chart_path):
                            with open(chart_path, 'rb') as img_file:
                                ...
                                logger.info(f"Graphique capturé: {groupe}")
                    
                    # Nettoyer
                    os.unlink(temp_file_path)
                    
                except Exception as e:
                    logger.error(f"Erreur capture graphique {groupe}: {e}")
            
            logger.info(f"Capture terminée: {len(chart_images)} graphiques générés")
            return chart_images
            
        except ImportError:
            logger.error("Html2Image non installé: pip install html2image")
            return {}
        except Exception as e:
            logger.error(f"Erreur Html2Image: {e}")
            return {}

    def _generate_single_chart_html(self, groupe, metier_details, index):
        """Génère HTML pour UN graphique spécifique avec données complètes"""
        return f"""
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="chart-container">
                <h3>{groupe}</h3>
                <canvas id="chart_{index}" width="500" height="180"></canvas>
            </div>
            
            <script>
                const metierDetails = {json.dumps(metier_details)};
                
                // COPIE EXACTE de la fonction JavaScript du main.js
                ...
                if (chartData) {{
                    ...
                    new Chart(canvas.getContext('2d'), {{
                        ...
                        options: {{
                            ...
                            animation: false,  // Pas d'animation pour la capture
                            plugins: {{
                                title: {{
                                    display: true,
